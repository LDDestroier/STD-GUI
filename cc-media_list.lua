...
local repoPath = tArg[2] or "cc-media"
	...
	while true do
		...
		if (event == "http_success") and (name and handle) then
			...
			if contents then
				...
				std.storeURLs[name] = {
					...
					url = "",
					creator = "???",
					description = "This piece of media is broken. Please contact LDDestroier on the forums and nag his ears off.",
					catagory = 12,
					forumPost = "n/a ",
		...
		elseif event == "http_failure" and name then
			...
			std.storeURLs[name] = {
				...
				url = "",
				creator = "???",
				description = "This piece of media cannot be downloaded. Please contact the developer and nag them.",
				catagory = 12,
				forumPost = "n/a ",
				keywords = {"fucking","error"},
			}
